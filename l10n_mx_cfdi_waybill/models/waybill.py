...
class Waybill(models.Model):
    ...
    cfdi_id = fields.Many2one('l10n_mx_cfdi.document', string='CFDI', ondelete='cascade', required=True)

    federal_highway_use = fields.Boolean(string='Uso de carretera federal', default=True)

    vehicle_id = fields.Many2one('l10n_mx_cfdi_waybill.vehicle', string='Vehículo')
    transporter_ids = fields.Many2many(
        'l10n_mx_cfdi_waybill.transporter', string='Contactos',
        relation='l10n_mx_cfdi_waybill_waybill_transporter_rel',
    )

    entry_ids = fields.One2many(
        'l10n_mx_cfdi_waybill.waybill_entry', 'waybill_id', string='Entradas', readonly=False
    )

    picking_ids = fields.Many2many(
        'stock.picking', string='Traslados',
        relation='stock_picking_waybill_rel',
    )

    @api.model
    def default_get(self, fields_list):
        res = super(Waybill, self).default_get(fields_list)
    ...
    def _onchange_picking_ids(self):
        ...
        if len(self.picking_ids.picking_type_id) > 1:
            raise ValidationError("Solo se pueden seleccionar traslados del mismo tipo")
    ...
    def action_post(self):
        ...
        try:
            ...
            self.message_post(
                body="Se ha publicado el CFDI de la carta porte",
        ...
        for transporter in self.transporter_ids:
            ...
            if transporter.type.code == '02' or transporter.type.code == '03':
                item_data['PartesTransporte'] = [
                    {"ParteTransporte": parte_transporte.code} for parte_transporte in transporter.parte_transporte_ids
                ]
    ...
    def _format_goods_and_locations_data(self):
        ...
        for idx, entry_id in enumerate(self.entry_ids):
            ...
            if entry_id.origin_address_id not in origin_locations_codes:
                ...
                departure_date = pytz.utc.localize(entry_id.departure_datetime).astimezone(tz)

                locations_data.append({
                    'TipoUbicacion': 'Origen',
                    'IDUbicacion': origin_location_id,
                    'RFCRemitenteDestinatario': self.issuer_id.vat,
                    'FechaHoraSalidaLlegada': departure_date.strftime('%Y-%m-%d %H:%M:%S'),
            ...
            if entry_id.destination_address_id not in destination_locations_codes:
                ...
                destination_locations_codes[entry_id.destination_address_id] = destination_location_id

                # get arrival date in user timezone
                tz = pytz.timezone(self.env.user.tz)
                arrival_date = pytz.utc.localize(entry_id.arrival_datetime).astimezone(tz)

                locations_data.append({
                    'TipoUbicacion': 'Destino',
                    'IDUbicacion': destination_location_id,
                    'RFCRemitenteDestinatario': self.issuer_id.vat,
                    'FechaHoraSalidaLlegada': arrival_date.strftime('%Y-%m-%d %H:%M:%S'),
                    'DistanciaRecorrida': entry_id.distance,
                    'Domicilio': self._format_address(entry_id.destination_address_id)
                })

        for idx, entry_id in enumerate(self.entry_ids):
            data = self._format_goods_transport_details(entry_id, origin_locations_codes, destination_locations_codes)
            goods_data.append(data)

        return goods_data, locations_data

    def _format_goods_transport_details(self, entry_id, origin_locations_codes, destination_locations_codes):
    ...
    def _format_address(self, location):
        c_pais = self.env['l10n_mx_catalogs.c_pais'].map_res_country(location.country_id)
        if c_pais.code == 'MEX':
            c_codigo_postal = self.env['l10n_mx_catalogs.c_codigo_postal'] \
                .search([('code', '=', location.zip)], limit=1)
            data = {
                'Pais': c_pais.code,
        ...
        for rec in self:
            if rec.federal_highway_use:
                if not rec.vehicle_id:
                    raise ValidationError('Debe seleccionar un vehículo para la carta porte')

                rec.vehicle_id.validate()

                if not rec.transporter_ids:
                    raise ValidationError('Debe seleccionar al menos un transportista en la sección "Contactos"')

                if not any(transporter.type.code == '01' for transporter in rec.transporter_ids):
                    raise ValidationError('Debe seleccionar al menos un transportista con tipo "Operador"')

                for transporter in rec.transporter_ids:
                    if not transporter.partner_id.vat:
                        raise ValidationError(
                            'Debe ingresar el RFC del contact: %s' % transporter.partner_id.name)

                    if not transporter.type:
                        raise ValidationError(
                            'Debe seleccionar el tipo de contacto para: %s' % transporter.partner_id.name)

                    if not transporter.driving_license:
                        raise ValidationError(
                            'Debe ingresar el número de licencia del contact: %s' % transporter.partner_id.name)
    ...
    def action_cancel(self):
        ...
        self.message_post(body='Carta Porte cancelada')
    ...
    def format_address_for_report(self, address):
        ...
        result += ' ' + address['NumeroExterior'] if address.get('NumeroExterior') else ''
        result += ' ' + address['NumeroInterior'] if address.get('NumeroInterior') else ''
        result += ', ' + address['Estado'] if address.get('Estado') else ''
        result += ', ' + address['Pais'] if address.get('Pais') else ''
        result += ', C.P. ' + address.get('CodigoPostal') if address.get('CodigoPostal') else ''
        return result
    ...
    def _assign_serial_number(self):
        ...
        self.cfdi_id.folio = re.match(r'CP/\d{4}/\d{2}/(\d+)', name).group(1)  # extract number as folio
...
class WaybillEntry(models.Model):
    ...
    waybill_id = fields.Many2one('l10n_mx_cfdi_waybill.waybill', 'Carta porte', ondelete='cascade')
    ...
    origin_address_id = fields.Many2one('res.partner', 'Origen', default=_compute_addresses_times_and_distance)
    destination_address_id = fields.Many2one('res.partner', 'Destino', default=_compute_addresses_times_and_distance)

    departure_datetime = fields.Datetime('Salida', default=_compute_addresses_times_and_distance)
    arrival_datetime = fields.Datetime('Arribo', default=_compute_addresses_times_and_distance)
    ...
    def create(self, vals_list):
        res = super(WaybillEntry, self).create(vals_list)
    ...
    def _get_defaults(self, picking_id):
        ...
        if picking_id:
            if picking_id.location_id.warehouse_id:
                res['origin_address_id'] = picking_id.location_id.warehouse_id.partner_id.id
            else:
                res['origin_address_id'] = picking_id.partner_id.id

            if picking_id.location_dest_id.warehouse_id:
                res['destination_address_id'] = picking_id.location_dest_id.warehouse_id.partner_id.id
            else:
    ...
    def _validate_required_fields(self):
        for entry in self:
            if not entry.origin_address_id:
                raise ValidationError("Debe seleccionar una dirección de origen para el traslado de bienes")

            if not entry.destination_address_id:
                raise ValidationError("Debe seleccionar una dirección de destino para el traslado de bienes")

            if entry.origin_address_id.country_id != entry.destination_address_id.country_id:
                raise UserError("La ubicación de origen y destino deben ser del mismo país.")

            if not entry.departure_datetime:
                raise ValidationError('Debe ingresar la fecha y hora de salida del traslado')

            if not entry.arrival_datetime:
                raise ValidationError('Debe ingresar la fecha y hora de llegada del traslado')

            if not entry.distance:
                raise ValidationError('Debe ingresar la distancia recorrida del traslado')

            product_id = entry.product_id
            if not product_id.l10n_mx_cfdi_product_code_id:
                raise ValidationError('Debe ingresar el código del producto en la ficha del producto: %s' %
                                      entry.product_id.name)

            if not product_id.l10n_mx_cfdi_product_measurement_unit_id:
                raise ValidationError(
                    'Debe ingresar la unidad de medida del producto en la ficha del producto: %s' %
                    entry.product_id.name)

            if not product_id.weight:
                raise ValidationError(
                    'Falta el campo Peso en: %s' % entry.product_id.name)

            if not entry.origin_address_id.zip:
                raise ValidationError(
                    'Falta el campo Código Postal en: %s' % entry.origin_address_id.name)
            ...
            if not entry.destination_address_id.zip:
                raise ValidationError(
                    'Falta el campo Código Postal en: %s' % entry.destination_address_id.name)
            ...
            if entry.move_id:
                ...
                for cn in related_cne.waybill_id:
                    if cn.state == 'published':
                        raise ValidationError(
                            'La línea de traslado de "%s" ya está incluido en la carta porte "%s"' %
                            (entry.move_id.name, cn.name)
                        )
    ...
    def _compute_arrival_datetime(self):
        for entry in self:
            if entry.departure_datetime and entry.duration:
                entry.arrival_datetime = entry.departure_datetime + timedelta(hours=entry.duration)
            else:
