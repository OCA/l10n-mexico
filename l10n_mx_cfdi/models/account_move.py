...
class AccountMove(models.Model):
    ...
    def _validate_invoice_cfdi_required_fields(self):
        ...
        if err_msg:
            raise ValidationError("No se puede generar el CFDI:\n" + err_msg)
    ...
    def _resolve_receiver_data_from_xml(self, namespaces, root):
        ...
        if not receiver_id:
            raise UserError(
                "No se encontró el receptor del certificado. RFC: %s"
                % receiver.attrib["Rfc"]
            )
    ...
    def _resolve_issuer_from_xml(self, namespaces, root):
        ...
        if not issuer_id:
            ...
            if not partner_id:
                raise UserError(
                    "No se encontró el partner del emisor del certificado. RFC: %s"
                    % issuer.attrib["Rfc"]
                )
    ...
    def action_generate_cfdi(self):
        ...
        if self.cfdi_document_id.state == "published":
            raise UserError("El CFDI ya ha sido publicado")
        ...
        if self.move_type == "out_refund":
            ...
            if self.amount_residual != 0:
                raise UserError(
                    "No se puede generar un CFDI de nota de "
...
class AccountMoveLine(models.Model):
    ...
    cfdi_price_unit = fields.Monetary(
        string="Precio Unitario sin Impuestos",
