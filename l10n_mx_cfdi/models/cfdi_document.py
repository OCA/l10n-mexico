...
class Document(models.Model):
    ...
    version = fields.Char(string="Version", default="4.0")
    ...
    files_in_cache = fields.Boolean(
        readonly=True, compute="download_files_if_needed", store=False
    )

    issuing_datetime = fields.Datetime(
        string="Fecha de emisión", readonly=True, compute="_load_json_data"
    )
    cert_number = fields.Char(
        string="Número de certificado", readonly=True, compute="_load_json_data"
    )
    original_string = fields.Char(
        string="Cadena original", readonly=True, compute="_load_json_data"
    )
    cfdi_signature = fields.Char(
        string="Firma del CFDI", readonly=True, compute="_load_json_data"
    )
    sat_signature = fields.Char(
        string="Firma del SAT", readonly=True, compute="_load_json_data"
    )
    sat_cert_number = fields.Char(
        string="Número de certificado del SAT", readonly=True, compute="_load_json_data"
    )
    rfc_prov_certif = fields.Char(
        string="RFC del proveedor de certificación",
        readonly=True,
        compute="_load_json_data",
    )
    signing_date = fields.Datetime(
        string="Fecha de timbrado", readonly=True, compute="_load_json_data"
    )
    ...
    tax_codes = fields.Char(
        string="Código de impuesto", readonly=True, compute="_load_json_data"
    )

    verification_url = fields.Char(
        string="URL de verificación", readonly=True, compute="_load_json_data"
    )
    verification_qr_code = fields.Binary(
        string="Código QR de Verificación", readonly=True, compute="_load_json_data"
    )
        ...
        for entry in self:
            if entry.tracking_id:
                if not entry.pdf_file:
                    ...
                    if report:
                        # force the report to be rendered to work around a bug in _render_qweb_pdf
                        report = report.with_context({"force_report_rendering": True})
                        doc_data, doc_format = report._render_qweb_pdf(resource_ids)
                        # in some scenarios, the report is not generated, so we need to check if the file is empty
                        if doc_data:
    ...
    def get_sequence_for_cfdi_type(self, vals_list):
        ...
        if vals_list["type"] == "I":
            ...
            raise UserError("Tipo de certificado desconocido")
    ...
    def cancel(self, reason: str, replacement=None, simulate=False):
        ...
        if not simulate:
            ...
            if (
                ...
                raise UserError("Error al cancelar el certificado: %s" % res["Message"])
        else:
            self.state = "canceled"
    ...
    def publish(self, cfdi_data):
        ...
        for entry in self:
            if entry.state != "draft":
                raise UserError("El certificado no está en borrador")
            ...
            if similar_certificates_count > 0:
                raise UserError(
                    "Ya existe un certificado publicado con la serie y folio indicados"
                )
            ...
            if res["Status"] == "active":
                ...
                raise UserError("Error al publicar el certificado: %s" % res["Message"])

    def action_cancel(self):
        self.ensure_one()

        return {
            "name": "Cancelar certificado",
    ...
    def action_get_cancellation_request_proof(self):
        ...
        if self.state != "canceled":
            raise UserError("El certificado no está cancelado")
