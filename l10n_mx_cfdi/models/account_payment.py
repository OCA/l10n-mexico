...
class AccountPayment(models.Model):
    ...
    def action_generate_cfdi(self):
        ...
        if self.cfdi_document_id:
            raise ValidationError("El pago ya tiene un CFDI asociado.")

        if not self.is_reconciled:
            raise ValidationError("El pago no está totalmente reconciliado.")
    ...
    def create_payment_cfdi(self):
        ...
        if self.move_type != "entry" or self.payment_type != "inbound":
            raise ValidationError("Solo se pueden crear pagos de tipo entrada.")
        ...
        for invoice in self.reconciled_invoice_ids:
            if not invoice.cfdi_document_id:
                raise ValidationError(
                    'Error al emitir CFDI tipo Comprobante de Pago. La factura "%s" no tiene CFDI'
                    % invoice.name
                )
