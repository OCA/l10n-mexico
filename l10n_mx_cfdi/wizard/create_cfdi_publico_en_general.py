...
class CFDIGenericInvoiceCreate(models.TransientModel):
    ...
    periodicity_id = fields.Many2one(
        "l10n_mx_catalogs.c_periodicidad", string="Periodicidad", required=True
    )
    ...
    def _validate_invoice(self, invoice):
        ...
        if invoice.state != "posted":
            raise ValidationError(
                'La factura "%s" factura que no este confirmada' % invoice.name
            )
        ...
        if related_cfdi:
            raise ValidationError(
                "La factura '%s' ya tiene un CFDI publicado" % invoice.name
            )
