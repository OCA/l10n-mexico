...
class HrExpenseSheet(models.Model):
    ...
    def _generate_supplier_payments(self):
        ...
        if not pay_journal:
            raise UserError(_("Debe configurar al menos un diario bancario para registrar pagos de proveedores."))
        ...
        if pay_journal.outbound_payment_method_line_ids:
            ...
            manual = self.env.ref('account.account_payment_method_manual_out', raise_if_not_found=False)
            if not manual:
                raise UserError(_(
                    "El diario '%s' no tiene método de pago y no existe el método manual.") 
                    % pay_journal.name
                )
        ...
        for sheet in self:
            for invoice in sheet.account_move_ids.filtered(lambda m: m.move_type=='in_invoice' and m.state=='posted' and m.payment_state!='paid'):
                ctx = {
    ...
    def _create_employee_reimbursement_invoice(self):
        ...
        if not acc_debit or not acc_credit:
            raise UserError(_(
                "Debes configurar en Ajustes → Empresas las cuentas de débito y crédito "
        ...
        for sheet in self:
            ...
            # Price_unit positivo (débito) y price_unit negativo (crédito) en la cuenta específica
            invoice_lines = [
                (0, 0, {
    ...
    def _reconcile_account_lines(self):
        ...
        for sheet in self:
            ...
            if len(move_lines) >= 2:
                ...
                wizard = ReconcileWizard.with_context(ctx).new({'allow_partials': True})
                wizard.reconcile()
