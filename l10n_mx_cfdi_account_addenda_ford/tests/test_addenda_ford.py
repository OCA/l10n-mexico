...
class TestAddendaFord(CFDIAccountTestCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_addenda_view_flag(self):
        ...
        self.assertEqual(self.addenda_view.name, "Addenda Ford")

    def test_partner_addenda_related_name(self):
        self.assertEqual(self.customer.l10n_mx_edi_addenda_name, "Addenda Ford")

    def test_ford_flag_compute(self):
        invoice = self._create_cfdi_invoice()
        self.assertTrue(invoice.ford_flag)
        ...
        self.assertFalse(invoice.ford_flag)

    def test_render_ford_addenda_via_framework(self):
        invoice = self._create_cfdi_invoice(ref="GSDB-001", ford_ref="ASN-42")
        sample = (
        ...
        self.assertIn(b"GSDB-001", result)
        self.assertIn(b"ASN-42", result)
        self.assertIn(b"fomadd", result)
